...
<body>
        ...
        <main class="chart-area">
            ...
            <div class="trade-columns">
                <div class="buy-column">
                    <p>Available USDT: <span id="available-usdt"><?php echo number_format($available_usdt, 6); ?></span></p>
                    <div id="maxBuyBTC">Max Buy BTC: 0.000000</div>
                    <form id="buy-form">
                        <span id="buy-price">Loading...</span>
                        
                        <label for="buy-amount">Amount (BTC):</label>
                        <input type="number" id="buy-amount" placeholder="Amount" value="0.000000" min="0" step="0.000001" oninput="validateAmount('buy')">
                ...
                <p>Available USDT: <span class="available-usdt"><?php echo number_format($available_usdt, 6); ?></span></p>
                <div id="maxSellBTC">Max Sell BTC: 0.000000</div>
                    <form id="sell-form">
                        <span id="sell-price">Loading...</span>

                        
                        <label for="sell-amount">Amount (BTC):</label>
                        <input type="number" id="sell-amount" placeholder="Amount" value="0.000000" min="0" step="0.000001" oninput="validateAmount('sell')">
                        <div>select period:</div>
                        <table class="option-table">
                            <tr>
                                <td><button type="button" class="time-option" onclick="selectTimeOption(this)">30s</button></td>
    ...
    function validateForm(event, formId) {
        ...
        if (amount < 0.0001 || amount > maxAmount) {
            event.preventDefault();
            alert(`Please enter an amount between 0.0001 BTC and the maximum allowed: ${maxAmount} BTC.`);
            return;
        }
    ...
    function startCountdown(overlay, timeRemaining, startPrice, amount, orderDirection) {
        ...
        const countdownInterval = setInterval(() => {
            ...
            if (timeRemaining <= 0) {
                ...
                showOrderConfirmation(symbol, amount, startPrice); // Pass order details
                saveOrderHistory(startPrice, amount, parseFloat(plElement.innerText.split('$')[1]));
            }
        }, 1000);
    }

    function showOrderConfirmation(symbol, amount, startPrice) {
        const confirmationOverlay = document.getElementById('order-confirmation-overlay');
        const endPrice = parseFloat(document.getElementById('realTimePrice').innerText.replace('Current Price: $', '')).toFixed(6);
        const profitLoss = ((endPrice - startPrice) * amount).toFixed(6);

        // Update the content of the confirmation overlay with order details
        ...
        confirmationOverlay.style.display = 'flex'; // Show the overlay

        // Save order details to the database
        saveOrderToDatabase(symbol, amount, startPrice, endPrice, profitLoss);

        document.getElementById('close-confirmation').onclick = () => {
            confirmationOverlay.style.display = 'none'; // Hide it when close is clicked
        };
    }

    function saveOrderToDatabase(symbol, amount, startingPrice, endPrice, expectedPL) {
        const orderData = {
            symbol: symbol,
            amount: amount,
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                // Update the user balance after saving the order
